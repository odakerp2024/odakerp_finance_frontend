...
@Component({
  selector: 'app-vsprealert',
  templateUrl: './vsprealert.component.html',
  styleUrls: ['./vsprealert.component.css']
})
export class VsprealertComponent implements OnInit {
    ...
    onclickNext() {
        ...
        if (checked == 0) {
            Swal.fire("Please check atleast one Principal.");
        }
        if (checked == 1) {
           var values = "VesselID: " + this.chkVessel + "&VoyageID:" + this.chkVoyage + "&PrincipalID:" + this.chkPrincipal + "&PODAgentID:" + this.chkAgent;
        var encrypted = this.ES.set(localStorage.getItem("EncKey"), values);
        this.router.navigate(['/views/exportmanager/vessellevel/vsprealert/prealertfinal/prealertfinal/'], { queryParams: { encrypted } });
        }
    }
