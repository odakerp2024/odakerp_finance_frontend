...
import { OpeningBalancesBankService } from 'src/app/services/financeModule/opening-balances-bank.service';
...
export class BankComponent implements OnInit {
  ...
  constructor(
    ...
    private OpeningBalancesBankService: OpeningBalancesBankService,
  ...
  async autoCodeGeneration(event: any) {
    if (event) {
      var Info = this.autoGenerateCodeList.filter(x => x.ObjectName == 'Bank');
  ...
  setPage(page: number) {
    if (this.PaymentBankList.length) {
      if (page < 1 || page > this.pager.totalPages) {
        return;
      }
      // get pager object from service
      this.pager = this.ps.getPagers(this.PaymentBankList.length, page);

      // get current page of items
      this.pagedItems = this.PaymentBankList.slice(this.pager.startIndex, this.pager.endIndex + 1);
    } else {
      this.pagedItems = [];
    }
  }
  getBankList() {
    const payload = this.accountBankForm.value
    this.OpeningBalancesBankService.GetBankList(payload).subscribe((result) => {
      if (result.message == "Success" && result.data.Table.length > 0) {
        this.PaymentBankList = result.data.Table;
        this.setPage(1);
      } else {
  ...
  openPopup() {
    this.getPermissionListForCreate(601);
  }
  ...
  updateAutoGenerated() {
    let Info = this.autoGenerateCodeList.filter(x => x.ObjectName == 'Bank');
    if (Info.length > 0) {
  ...
  save() {
    ...
    if (uploadForm.FileName == '' || uploadForm.FileName == undefined || uploadForm.FileName == null) {
      ...
      this.OpeningBalancesBankService.upload(employeeMaster).subscribe((result: any) => {
        if (result.message == "Success") {
          this.file = '';
          ...
          this.OpeningBalancesBankService.uploadTransJournal({}).subscribe((result: any) => {
            if (result.message === "Success") {
