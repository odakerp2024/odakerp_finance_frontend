...
import { OpeningBalancesAssetService } from 'src/app/services/financeModule/opening-balances-asset.service';
...
export class AssetComponent implements OnInit {
  ...
  constructor(
    ...
    private OpeningBalancesAssetService: OpeningBalancesAssetService,
  ...
  setPage(page: number) {
    if (this.PaymentAssetList.length) {
      if (page < 1 || page > this.pager.totalPages) {
        return;
      }
      // get pager object from service
      this.pager = this.ps.getPagers(this.PaymentAssetList.length, page);

      // get current page of items
      this.pagedItems = this.PaymentAssetList.slice(this.pager.startIndex, this.pager.endIndex + 1);
    } else {
      this.pagedItems = [];
    }
  }
  getPaymentAssetList() {
    const payload = this.accountAssetForm.value
    this.OpeningBalancesAssetService.GetAssetList(payload).subscribe((result) => {
      if (result.message == "Success" && result.data.Table.length > 0) {
        this.PaymentAssetList = result.data.Table;
        this.setPage(1);
      } else {
        this.PaymentAssetList = []
        this.pagedItems = [];
      }
    })
  }
  ...
  openPopup() {
    debugger
    this.getPermissionListForCreate(613);
  }
  ...
  editDivision(data: any) {
    let value = data
    this.getPermissionListForUpdate(613, value);
  }
  search() {
    this.getPaymentAssetList()
  }
  ...
  async autoCodeGeneration(event: any) {
    if (event) {
      var Info = this.autoGenerateCodeList.filter(x => x.ObjectName == 'Asset');
      ...
      if (Info.length > 0) {
        ...
        Swal.fire('Please create the auto-generation code for Asset.')
      }
  ...
  updateAutoGenerated() {
    let Info = this.autoGenerateCodeList.filter(x => x.ObjectName == 'Asset');
    if (Info.length > 0) {
  ...
  save() {
    ...
    if (uploadForm.FileName == '' || uploadForm.FileName == undefined || uploadForm.FileName == null) {
      ...
      this.OpeningBalancesAssetService.upload(employeeMaster).subscribe((result: any) => {
        if (result.message == "Success") {
          this.file = '';
          ...
          this.getPaymentAssetList();
          this.OpeningBalancesAssetService.uploadTransJournal({}).subscribe((result: any) => {
            if (result.message === "Success") {
      ...
      if (data.length > 0) {
        ...
        if (data[0].SubfunctionID == paylod.SubfunctionID) {
          ...
          if (data[0].Update_Opt == 2) {
            ...
            this.router.navigate(['/views/transactions/openingBalances/asset-detail', { id: id }])
