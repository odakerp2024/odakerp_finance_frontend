...
import { OpeningBalancesLiabilityService } from 'src/app/services/financeModule/opening-balances-liability.service';
...
export class LiabilityComponent implements OnInit {
  ...
  constructor(
    ...
    private PaymentLiabilityService: OpeningBalancesLiabilityService,
  ...
  setPage(page: number) {
    if (this.PaymentLiabilityList.length) {
      ...
      this.pager = this.ps.getPagers(this.PaymentLiabilityList.length, page);
  ...
  getPaymentliabilityList() {
    const payload = this.accountLiabilityForm.value
    this.PaymentLiabilityService.GetLiabilityist(payload).subscribe((result) => {
      if (result.message == "Success" && result.data.Table.length > 0) {
        this.PaymentLiabilityList = result.data.Table;
        this.setPage(1);
      } else {
        this.PaymentLiabilityList = []
        this.pagedItems = [];
      }
    })
  }
  ...
  editDivision(data: any) {
    this.getPermissionListForUpdate(603, data);
  }
  search() {
    this.getPaymentliabilityList()
  }
  ...
  updateAutoGenerated() {
    let Info = this.autoGenerateCodeList.filter(x => x.ObjectName == 'Liability');
    if (Info.length > 0) {
  ...
  async autoCodeGeneration(event: any) {
    ...
    if (event) {
      var Info = this.autoGenerateCodeList.filter(x => x.ObjectName == 'Liability');
  ...
  save() {
    ...
    if (uploadForm.FileName == '' || uploadForm.FileName == undefined || uploadForm.FileName == null) {
      ...
      this.PaymentLiabilityService.upload(employeeMaster).subscribe((result: any) => {
        if (result.message == "Success") {
          this.file = '';
          ...
          this.PaymentLiabilityService.uploadTransJournal({}).subscribe((result: any) => {
            if (result.message === "Success") {
      ...
      if (data.length > 0) {
        ...
        if (data[0].SubfunctionID == paylod.SubfunctionID) {
          ...
          if (data[0].Update_Opt == 2) {
            ...
            this.router.navigate(['/views/transactions/openingBalances/liability-detail', { id: id }])
