...
import { ExpensesService } from 'src/app/services/financeModule/expenses.service';
...
export class ExpensesComponent implements OnInit {
  ...
  setPage(page: number) {
    if (this.PaymentExpensesList.length) {
      if (page < 1 || page > this.pager.totalPages) {
        return;
      }
      // get pager object from service
      this.pager = this.ps.getPagers(this.PaymentExpensesList.length, page);

      // get current page of items
      this.pagedItems = this.PaymentExpensesList.slice(this.pager.startIndex, this.pager.endIndex + 1);
    } else {
      this.pagedItems = [];
    }
  }
  getExpensesList() {
    const payload = this.accountExpensesForm.value
    this.ExpensesService.GetExpensesList(payload).subscribe((result) => {
      if (result.message == "Success" && result.data.Table.length > 0) {
        this.PaymentExpensesList = result.data.Table;
        this.setPage(1);
      } else {
        this.PaymentExpensesList = []
        this.pagedItems = [];
      }
    })
  }
  ...
  openPopup() {
    debugger
    this.getPermissionListForCreate(599);
  }
  ...
  editDivision(data: any) {
    this.getPermissionListForUpdate(599, data);
  }
  search() {
    this.getExpensesList();
  }
  ...
  updateAutoGenerated() {
    ...
    let Info = this.autoGenerateCodeList.filter(x => x.ObjectName == 'Expenses');
    if (Info.length > 0) {
  ...
  async autoCodeGeneration(event: any) {
    if (event) {
      var Info = this.autoGenerateCodeList.filter(x => x.ObjectName == 'Expenses');
  ...
  save() {
    ...
    if (uploadForm.FileName == '' || uploadForm.FileName == undefined || uploadForm.FileName == null) {
      ...
      this.ExpensesService.upload(employeeMaster).subscribe((result: any) => {
        if (result.message == "Success") {
          this.file = '';
          ...
          this.getExpensesList();
          this.ExpensesService.uploadTransJournal({}).subscribe((result: any) => {
            if (result.message === "Success") {
  ...
  getPermissionListForUpdate(value, id) {
    ...
    // Check Permission for Expenses Add
    const userID = localStorage.getItem("UserID");
    const paylod = {
      userID: Number(userID),
      Ref_Application_Id: "4",
      ...
      if (data.length > 0) {
        ...
        if (data[0].SubfunctionID == paylod.SubfunctionID) {
          ...
          if (data[0].Update_Opt == 2) {
            ...
            this.router.navigate(['/views/transactions/openingBalances/expenses-detail', { id: id }])
