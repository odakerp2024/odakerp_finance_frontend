...
@Component({
  selector: 'app-gstinput-register',
  templateUrl: './gstinput-register.component.html',
  styleUrls: ['./gstinput-register.component.css']
})
export class GSTInputRegisterComponent implements OnInit {
  ...
  getGSTReportList() {
    ...
    this.FNReportService.getGSTInputRegister(this.reportFilter.value).subscribe(result => {
      this.reportList = [];
  ...
  getGSTOutputRegisterExcel() {
    ...
    this.FNReportService.getGSTInputRegisterExcel(this.startDate,this.endDate).then(x => {
      var newBlob = new Blob([x], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", });
      const data = window.URL.createObjectURL(newBlob);
      saveAs(data, "GSTInputRegisterExcel.xlsx");
     // window.open(data);
