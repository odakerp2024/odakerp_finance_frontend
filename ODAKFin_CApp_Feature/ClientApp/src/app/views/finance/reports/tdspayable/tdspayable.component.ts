...
@Component({
  selector: 'app-tdspayable',
  templateUrl: './tdspayable.component.html',
  styleUrls: ['./tdspayable.component.css']
})
export class TDSPayableComponent implements OnInit {
  ...
  getTDSPayableReport() {
    ...
    this.FNReportService.getTDSPayableReport(this.reportFilter.value).subscribe(result => {
      this.reportList = [];
  ...
  getTDSPayableExcel() {
    ...
    this.FNReportService.getTDSPayableExcel(this.startDate,this.endDate).then(x => {
      var newBlob = new Blob([x], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", });
      const data = window.URL.createObjectURL(newBlob);
      saveAs(data, "TDSPayableExcel.xlsx");
     // window.open(data);
