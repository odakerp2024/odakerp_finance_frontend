...
@Component({
  selector: 'app-cntrpickupdrop',
  templateUrl: './cntrpickupdrop.component.html',
  styleUrls: ['./cntrpickupdrop.component.css']
})
export class CntrpickupdropComponent implements OnInit {
    CntrPickDrop: FormGroup;
    ...
    Pickdropallvalues: CntrPickDrop[];
    StorageLocAllvalues: CntrPickDrop[];
    ...
    onInitalBinding() {
        ...
        this.es.getPrincibleList().subscribe(data => {
            this.fillPricibleMasterAllvalues = data;
        }, (error: HttpErrorResponse) => {
            Swal.fire(error.message);
        });
        this.es.getCustomerList().subscribe(data => {
            this.CustomerMasterAllvalues = data;
        }, (error: HttpErrorResponse) => {
            Swal.fire(error.message);
        });
        this.service.getPickDropTypeList().subscribe(data => {
            this.Pickdropallvalues = data;
        }, (error: HttpErrorResponse) => {
            Swal.fire(error.message);
        });
        this.service.getStorageLocList().subscribe(data => {
            this.StorageLocAllvalues = data;
        }, (error: HttpErrorResponse) => {
            Swal.fire(error.message);
        });
    ...
    createForm() {
        ...
        this.CntrPickDrop = this.fb.group({
            ID: 0,
            OfficeLocation: 0,
            Reference: '',
            RefDate: '',
