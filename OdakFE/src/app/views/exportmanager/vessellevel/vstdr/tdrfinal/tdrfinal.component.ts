...
@Component({
  selector: 'app-tdrfinal',
  templateUrl: './tdrfinal.component.html',
  styleUrls: ['./tdrfinal.component.css']
})
export class TdrfinalComponent implements OnInit {
    ...
    onclickNext() {
        ...
        if (checked == 0) {
            Swal.fire("Please check a BOL Number");
        }
        else {
            var values = "VesselID: " + this.VesselID + "&VoyageID:" + this.VoyageID + "&PrincipalID:" + this.PrincipalID + "&PODAgentID:" + this.PODAgentID;
            var encrypted = this.ES.set(localStorage.getItem("EncKey"), values);
            this.router.navigate(['/views/exportmanager/vessellevel/vstdr/tdrmail/tdrmail/'], { queryParams: { encrypted } });
        }
    }
